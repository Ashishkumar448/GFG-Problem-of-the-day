...
class Solution {
    public boolean isBridge(int V, int[][] edges, int c, int d) {
        ...
        // Step 2: DFS visit before removing the edge
        boolean[] visited = new boolean[V];
        dfs(c, adj, visited, c, d);  // pass c and d to avoid that edge

        // If we still reach d, then (c,d) is not a bridge
        return !visited[d];
    }

    private void dfs(int u, List<List<Integer>> adj, boolean[] vis, int c, int d) {
        vis[u] = true;
        for (int v : adj.get(u)) {
